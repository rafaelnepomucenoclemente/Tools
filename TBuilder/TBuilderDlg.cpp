							    ...
							    CFile *pTFILE)
{
	...
	pTFILE->Write( &wINDEX, sizeof(WORD));
	for( i=0; i<nCount; i++)
	...
	for( i=0; i<nCount; i++)
		if(pItem->m_pHugeMAP->m_pENABLE[i])
		{
			...
			pItem->LoadUNIT( &pSHADE, &pSKIN, &pTILE, &vSHADOW, pREGION, &vecDETAIL, pDETAIL, pHEIGHT, i, FALSE);
			pItem->LoadOBJ(
				&mapOBJ,
				&mapSFX,
				&mapSND,
			...
			for( int j=0; j<nSIZE; j++)
				unit.Write( &pDETAIL[j], sizeof(BYTE));
			...
			// DETAIL INDEX.
			for( int j=0; j<vecDETAIL.size(); ++j )
				if( vDETAIL[ vecDETAIL[ pDETAIL[j] ] ] == TILE_NULL )
					vecDETAIL[ pDETAIL[j] ] = TILE_NULL;

			nSIZE = (int)vecDETAIL.size() * sizeof(WORD);
			unit.Write( &nSIZE, sizeof(int));
			...
			if( nSIZE > 0 )
			{
				for( int j=0; j<vecDETAIL.size(); ++j )
				{
					WORD wDETAIL = vecDETAIL[j];
					unit.Write( &wDETAIL, sizeof(WORD));
				}
			}
			vecDETAIL.clear();
			// DETAIL INDEX.
