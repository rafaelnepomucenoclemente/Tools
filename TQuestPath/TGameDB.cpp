...
void CTGameDB::WriteDATA( CArchive& vAR)
{
	MAPTNDATA::iterator itTNDATA;

	DWORD dwCount = DWORD(m_mapTNDATA.size());
	vAR << dwCount;
	...
	dwCount = DWORD(m_vTMDATA.size());
	vAR << dwCount;

	for( DWORD i=0; i<dwCount; i++)
	{
		vAR << m_vTMDATA[i]->m_wMonID
			<< m_vTMDATA[i]->m_wKind
			<< m_vTMDATA[i]->m_strTITLE
			<< m_vTMDATA[i]->m_strNAME
			<< m_vTMDATA[i]->m_wMapID
			<< m_vTMDATA[i]->m_fPosX
			<< m_vTMDATA[i]->m_fPosY
			<< m_vTMDATA[i]->m_fPosZ;
	}
...
void CTGameDB::ReadDATA( CArchive& vAR)
{
	DWORD dwCount = 0;
	vAR >> dwCount;
	...
	for( DWORD i=0; i<dwCount; i++)
	{
		...
		m_mapTNDATA.insert( MAPTNDATA::value_type( pTNDATA->m_wID, pTNDATA));
	}
	vAR >> dwCount;
