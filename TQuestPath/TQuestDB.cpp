...
void CTQuestDB::ReleaseDATA()
{
	...
	ClearMAPVDWORD(&m_mapTMONITEMDATA);
	ClearMAPVDWORD(&m_mapTNPCITEMDATA);
	m_mapTQDATA.clear();
	m_mapTCDATA.clear();
}

void CTQuestDB::ClearMAPVDWORD( LPMAPVDWORD pTDATA)
{
	MAPVDWORD::iterator it;

	for( it = pTDATA->begin(); it != pTDATA->end(); it++)
	{
		(*it).second->clear();
		delete (*it).second;
	}
...
void CTQuestDB::MergeVDWORD( LPMAPVDWORD pTFROM,
							 LPMAPVDWORD pTINTO)
{
	MAPVDWORD::iterator itVDWORD;

	for( itVDWORD = pTFROM->begin(); itVDWORD != pTFROM->end(); itVDWORD++)
	{
		LPVDWORD pVID = (*itVDWORD).second;

		if( pVID && !pVID->empty() )
		{
			LPVDWORD pVTO = FindVDWORD(
				pTINTO,
				(*itVDWORD).first);
			...
			{
				pVTO = new VDWORD();
				pTINTO->insert( MAPVDWORD::value_type( (*itVDWORD).first, pVTO));
			}

			for( int i=0; i<INT(pVID->size()); i++)
				if( mapID.find((*pVID)[i]) == mapID.end() )
					mapID.insert( MAPDWORD::value_type( (*pVID)[i], (*pVID)[i]));

			for( i=0; i<INT(pVTO->size()); i++)
				if( mapID.find((*pVTO)[i]) == mapID.end() )
					mapID.insert( MAPDWORD::value_type( (*pVTO)[i], (*pVTO)[i]));

			MAPDWORD::iterator itID;
			pVTO->clear();

			for( itID = mapID.begin(); itID != mapID.end(); itID++)
				pVTO->push_back((*itID).second);
			mapID.clear();
		}
	}
}
